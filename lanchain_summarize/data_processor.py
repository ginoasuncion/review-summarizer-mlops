...
llm = OpenAI(
    temperature=0, model="gpt-3.5-turbo-instruct", openai_api_key=openai_api_key
)

# Load fake data from CSV
df = pd.read_csv(
    "C:/Users/kerel/review-summarizer-mlops/lanchain_summarize/fake_video_data.csv"
)
...
def summarize_shoe_review(model_name):
    try:
        # Find the row where the title contains the model name (case-insensitive)
